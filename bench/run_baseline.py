...
def _logs_dir() -> Path:
    # repo/plc_logger/bench/run_baseline.py -> repo/plc_logger/agent/plc_agent/api/logs
    return Path(__file__).resolve().parents[1] / "agent" / "plc_agent" / "api" / "logs"


def _bench_metrics_path() -> Path:
    return _logs_dir() / "bench_metrics.csv"


def _bench_metrics_paths() -> List[Path]:
    base = _logs_dir()
    return [
        base / "bench_metrics.csv",
        base / "bench_metrics_pending.csv",
    ]


def _summary_path() -> Path:
    return _logs_dir() / "bench_baseline_summary.csv"


def _baseline_log_path() -> Path:
    return _logs_dir() / "bench_baseline_run.log"
...
def summarize_job(job_id: str) -> Dict[str, Any]:
    ...
    if not any(p.exists() for p in paths):
        raise RuntimeError(f"bench_metrics.csv not found in {paths[0].parent}")
    for path in paths:
        if not path.exists():
            continue
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run baseline benchmarks across table/column scenarios.")
    ...
    logger.info(
        "Baseline start base_url=%s device_id=%s db_target_id=%s cols=%s tables=%s interval_ms=%s run_minutes=%s",
        args.base_url,
        args.device_id,
        args.db_target_id,
        args.cols,
