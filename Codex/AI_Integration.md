...
**Phase 1 (MVP):** Hermes runtime + tool surface + session API + KB document pipeline (GLM-OCR) + seed KB from top 5 MFM models + Layer 1 rule-based validation + Layer 2 anomaly detection (cold start on synthetic data). Mode 1: "I know my setup" — engineer describes their site, Hermes configures everything through the existing pipeline. Engineer can also upload a new datasheet and Hermes learns the model on the spot. Anomaly NN runs on every configuration to cross-validate readings beyond static rules.

**Phase 2:** Skills system (learn, store, retrieve, export/import) + Mode 2: network discovery + Layer 2 warm-up from real installation data (cross-deployment training dataset export).

**Phase 3:** Layer 3 predictive runtime (statistical baseline → LSTM/TCN upgrade) + frontend chat UI + SLD topology inference + skill marketplace across deployments.
